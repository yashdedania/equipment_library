			...
			if($conn){
    			...
    			if(mysqli_num_rows($retval) > 0){		
					while($row = mysqli_fetch_assoc($retval)){
						...
						echo "<tr onclick=\"details('".$pid."','".$name."','".$class."','".$contact."','".$item."','".$qty."','".$p_key."',0,'".$reason."');\">
							<td>$count</td>
							<td>$pid</td>
							<td>$name</td>
							<td>$class</td>
			...
			<div id="myDialog" class="dialog-box">
				...
				<div class="box-content">
				    ...
				    <div class="box-body" style="height:200px; overflow:scroll;">
				      ...
				      <p id="approve" style="position:relative; left:80%; font-weight:bold; cursor:pointer; text-decoration:underline; padding:10px; width:5%;">Returned</p>
				      <input type="text" placeholder="ENTER A FLAG" style="position:absolute; left:10%; bottom:10%; width:60%;" id="flag" required="required" />
				      <p id="result" style="position:absolute; left:70%; top:1%;"></p>
				    </div>
			  	</div>
			</div>
