			...
			if($conn){
    			...
    			if(mysqli_num_rows($retval) > 0){		
					while($row = mysqli_fetch_assoc($retval)){
						$count++;
						$pid=$row['pid']; $date_taken=$row['date_taken']; $date_return=$row['date_return']; $status=$row['status']; $name=$row['name']; $class=$row['class']; $contact=$row['contact']; $p_key=$row['p_key']; $flag=$row['flag']; $email = $row["email"]; $reason = $row["reason"];
						...
						echo "<tr onclick=\"details('".$pid."','".$name."','".$class."','".$contact."','".$item."','".$qty."','".$primary."',2,'".$reason."');\">
							<td>$count</td>
							<td>$pid</td>
							<td>$name</td>
							<td>$class</td>
			...
			<div id="myDialog" class="dialog-box">
				...
				<div class="box-content">
				    ...
				    <div class="box-body" style="height:200px; overflow:scroll;">
				      ...
				      <!-- <p id="approve" style="position:relative; left:50%; display:block; cursor:pointer; border:1px solid black; border-radius:5px; padding:10px; width:5%;">Approve</p>
